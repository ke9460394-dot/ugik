...
local coordinateTracker = Tabs.TYTab:Paragraph({
    Title = "坐标追踪器",
    Desc = "实时显示当前位置坐标",
    Image = "map-pin",
    ImageSize = 20,
...
coordLabel.Parent = coordinateTracker.DescFrame

-- 优化点1：降低更新频率（每0.1秒更新一次，约10次/秒，视觉上无明显延迟）
local updateInterval = 0.1
local lastUpdateTime = 0

-- 优化点2：缓存上一次坐标，位置不变时不更新文本
local lastPos = Vector3.new()

local function updateCoordinates(deltaTime)
    -- 累计时间，达到间隔才更新
    ...
    if humanoidRootPart and humanoidRootPart:IsA("BasePart") then
        local pos = humanoidRootPart.Position
        -- 位置变化超过0.01时才更新（过滤微小抖动）
        if (pos - lastPos).Magnitude > 0.01 then
            lastPos = pos
            coordLabel.Text = string.format("X: %.2f\nY: %.2f\nZ: %.2f", pos.X, pos.Y, pos.Z)
        end
    else
        coordLabel.Text = "等待角色加载..."
    end
end

-- 用RenderStepped并传入deltaTime，方便控制更新频率
