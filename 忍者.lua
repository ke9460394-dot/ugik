...
DELTA["8"]["Text"] = [[通过完成密钥系统访问Delta，不需要很长时间!]]
...
DELTA["56"]["Text"] = [[选择是否执行、在新选项卡中打开等..]]
...
DELTA["c0"]["Text"] = ""
DELTA["c0"]["PlaceholderText"] = "-- 把你的脚本放在这里 ^_^"
...
DELTA["c5"]["Text"] = [[自动执行AutoExec文件中的脚本.]]
DELTA["c5"]["Name"] = [[Desc]]
...
DELTA["d8"]["Text"] = [[自动执行AutoExec文件中的脚本.]]
DELTA["d8"]["Name"] = [[Desc]]
...
DELTA["fb"]["Text"] = [[请在下面选择您想要的Delta主题]]
DELTA["fb"]["Name"] = [[Desc]]
...
DELTA["121"]["Text"] = [[一个专门提供开发所需物品的管理脚本]]
...
DELTA["172"]["Text"] = [[从Roblox控制台获取输出并将其显示在此菜单中的控制台.]]
...
DELTA["195"]["Text"] = [[控制台，提供Delta API的输出和输入.]]
	...
	function Verify()
				...
				if (#key > 0) then
					...
					if redeemResponse.StatusCode == 200 then
						if string.find(redeemResponse.Body, "true") then
							DELTA["18"]["Text"] = "已成功兑换密钥!"
							return true
						end
					end
				end;
				DELTA["18"]["Text"] = "检测到无效密钥，请重试!"
				return false
			end
		...
		elseif response.StatusCode == 429 then
			if not rateLimit then
				...
				task.spawn(function()
					while rateLimit do
						DELTA["18"]["Text"] = "您正处于限速状态，请减速。输入后再试一次 " .. rateLimitCountdown .. " 秒."
						wait(1)
						rateLimitCountdown = rateLimitCountdown - 1;
						if rateLimitCountdown < 0 then
							rateLimit = false;
							rateLimitCountdown = 0;
							DELTA["18"]["Text"] = "费率限制已过，请再试一次."
						end
					end
				end)
			end
		elseif response.StatusCode == 500 then
			errorWait = true;
			task.spawn(function()
				DELTA["18"]["Text"] = "服务器出现错误，请等待3秒后重试."
				wait(3)
				errorWait = false
			end)
		end
...
local function C_19c()
	...
	UILib:AddTab()
	UILib.Settings:AddSwitch("自动执行", "切换AutoExec文件夹中脚本的自动执行", not isfile("disableautoexec"), function(state)
		if (state) then
	...
	UILib.Settings:AddSwitch("自动关闭", "切换X按钮要求关闭弹出窗口", not isfile("preventautoclose"), function(state)
		if (state) then
	...
	UILib.Settings:AddSwitch("Syn 环境", "使用Synapse X的命名标准", isfile("uses_syn"), function(state)
		if (state) then
	...
	UILib.Settings:AddSwitch("控制台", "Toggle roblox console logs in the GUI", isfile("useconsole"), function(state)
		if (state) then
	...
	UILib.Settings:AddDropdown("FPS上限", "更改FPS上限以获得更流畅的体验", "60 FPS", {
		"60 FPS",
		"120 FPS",
		"Max FPS"
	}, function(selection)
	...
	UILib.Settings:AddDropdown("图标大小", "更改浮动图标的大小", readfile("iconsize"), {
		"中号",
		"小号",
		"大号"
	}, function(selection)
	...
	UILib.Settings:AddDropdown("图标形状", "更改浮动图标的形状", readfile("iconshape"), {
		"松鼠",
		"圆圈",
		"广场"
	}, function(selection)
	...
	UILib.Settings:AddDropdown("图标颜色", "更改浮动图标的颜色", readfile("iconcolor"), {
		"蓝色",
		"绿色",
		"紫色"
	}, function(selection)
	...
	UILib.Settings:AddButton("关注我", "复制我的b站号", function()
		setclipboard("UID:1531514159")
	end)
	UILib.Settings:AddButton("重新加入", "重新加入当前服务器", function()
		game:GetService("TeleportService"):Teleport(game.PlaceId, game:GetService("Players").LocalPlayer)
	end)
	UILib.Settings:AddButton("小型服务器", "加入玩家数量较低的服务器", function()
		local Http = game:GetService("HttpService")
	...
	UILib.Settings:AddButton("Serverhop", "传送到新服务器", function()
